...
def mintyper2_pipeline(args):
    ...
    if args.nanopore != []:
        for file in args.nanopore:
            ...
            cmd = 'kma -i {} -o {}/{} -t_db /home/people/malhal/mintyper2/consensus_genes_db -ID 50 -md 5 -mct 0.5 -t 8 -mem_mode -dense -ref_fsa -ont'.format(file, args.output, name)
            os.system(cmd)
    if args.illumina != []:
...
def find_common_genes(directory_path):
    ...
    if gene_lists:
        ...
        for gene_list in gene_lists:
            left_overs.update(set(gene_list) - common)
        left_overs_list = list(left_overs)
        left_overs_list.sort()
        return common, left_overs_list
    else:
